...
- 以 TCP 客户端身份**主动连接** Windows 服务器（统一拓扑：Windows 作服务器）
- 连接目标支持：USB（`adb reverse` 本机端口 `127.0.0.1:5000`）、局域网自动发现、手动输入 IP/端口
- 断线后**自动重连**，不计代价保证连接稳定（重连循环不因异常退出，直到用户停止服务）
- 解析协议头与消息体
...
统一拓扑：**Windows 作 TCP 服务器，Android 作 TCP 客户端**。

- ADB 模式：Windows 监听 `0.0.0.0:5000`（含 `adb reverse`），Android 连接设备本地 `127.0.0.1:5000`
- LAN 模式：Windows 同时监听 `0.0.0.0:5000` 与 `0.0.0.0:<LanListenPort>`（默认 `6000`），Android 连接发现的 `WindowsIP:6000` 或手动输入地址
- Android 连接成功后解析协议并播放，断线自动重连
- 连接目标持久化：手动连接的局域网服务器会保存到本地，下次启动 / 主页重连优先自动使用
...
- 生命周期管理
- 主动连接 Windows 服务器（连接目标解析）
- 断线自动重连（自动重连循环，异常不退出）
- 局域网发现（发送探测广播、接收应答、维护服务器列表）
- 持有播放控制器
- 持续运行于后台
- 通知栏状态更新
...
- Socket 接收线程：阻塞读取客户端 Socket `InputStream`
- 连接 / 重连协程：主动连接与断线自动重连
...
- 停止或重置 `AudioTrack`
- 更新状态为“正在自动重连”
- 按固定间隔自动重连 Windows 服务器（重连循环不因异常退出，直到用户显式停止服务）
...
- 作为 TCP 客户端连接 Windows 服务器（USB reverse / 局域网发现 / 手动输入）
- 断线自动重连
- 读取固定头
- 校验协议包
...
- 能启动前台服务
- 能在后台继续运行
- 能主动连接 Windows 服务器（USB / 局域网）并断线自动重连
- 能解析 `SessionInit`
